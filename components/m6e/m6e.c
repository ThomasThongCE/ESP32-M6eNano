...
void hwInit()
{
    ...
    ret = TMR_paramGet(rp, TMR_PARAM_GEN2_SESSION, &temp);
    ...
    ret = TMR_paramGet(rp, TMR_PARAM_GEN2_TARGET, &temp);
    checkerr(rp, ret, "Getting target");
    printf ("target: %d \r\n", temp);
    ...
    // hopfreq.max = numberof(valueList);
    // hopfreq.list = valueList;
